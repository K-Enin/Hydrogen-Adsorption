...
# Set negative mass values equal 0, problem of IWES model at the beginning
# Cut model to required simulation time (instead of 3h) 
sum_ELY_massflow_mol = np.where(sum_ELY_massflow_mol_old < 0, 0, sum_ELY_massflow_mol_old)
sum_ELY_massflow_mol = sum_ELY_massflow_mol[0:int(time_steps/10)] #dt_IWES/dt = 10
...
if plot_graph == True:
    ...
    fig, ax = plt.subplots()
    ax.plot(X_s[4,:], label = '2 cm') 
    ax.plot(X_s[8,:], label = '4 cm') 
    ax.plot(X_s[N,:], label = '12 cm')   
    plt.xlabel('time [min]')
    ...
    fig, ax = plt.subplots()
    ax.plot(X_f[4,:], label = '2 cm') 
    ax.plot(X_f[8,:], label = '4 cm') 
    ax.plot(X_f[N,:], label = '12 cm')   
    plt.xlabel('time [min]')
    ...
    # temperature T
    fig, ax = plt.subplots()
    ax.plot(T[4,:], label = '2 cm') 
    ax.plot(T[8,:], label = '4 cm') 
    ax.plot(T[N,:], label = '12 cm') 
    plt.xlabel('time [min]')
    plt.ylabel('$T$ [K]')
    ax.set_xticks(xticks, labels=labels)
