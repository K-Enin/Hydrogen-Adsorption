...
## column parameter
H = 0.1                 # height of column [m]
d_column = 0.3          # diameter of column [m]
dz = H/(N+2-1)          # height of one section [m]
...
u_leer_vektor_old = mass_flow_m3s/(np.pi*(d_column/2)**2)                  ## [m/s]

# interpolate, as IWES time step is larger than our time step
x = np.linspace(0, time_steps, u_leer_vektor_old.size)
interpolated_function = interp1d(x, u_leer_vektor_old, kind='linear')
new_x = np.linspace(x.min(), x.max(), time_steps)
u_leer_vektor = interpolated_function(new_x)

# Check if CFL condition is satisfied
if u_leer_vektor.any() < dz/dt: 
...
d_p_makro = 300e-9      # macropore diameter [m] (Mette)
...
u_leer_matrix = np.full((N+2, time_steps), np.nan)
for i in range(0,N+2):
    u_leer_matrix[i,i:] = u_leer_vektor[:time_steps-(i)]

# Replace all NaN with u_leer[0]
u_leer_matrix[np.isnan(u_leer_matrix)] = u_leer_vektor[0]

# p=3e6 is sufficient, as only very small pressure losses due to low flow velocity
p_ergun = 3e6*np.ones((N+2, time_steps)) 

if p_ergun_qu == True:
    # effective viscosity of hydrogen
    eta_eff = eta_f*2.0*np.exp(3.5e-3*u_leer_matrix*d_p*rho_f_in/eta_f)

    # Calculate pressure loss at matrix multiplication, long calculation times
    dp = dz*(150*(1-eps)**2/eps**3*(eta_eff/d_p**2)*u_leer_matrix+1.75*(1-eps)/eps**3*rho_f_in/d_p*u_leer_matrix**2)
...
# In line 245, you can choose interpolated version
beta = 0.207e-3

# effective axial thermal conductivity
Pe_lambda = u_leer_matrix*d_p*rho_f_in*cp_f/lambda_f
...
for i in range(0, time_steps-1):    
    ...
    # Knudsen-diffusion
    D_Kn = (4/3)*d_p_makro*np.sqrt(R*T[:,i]/(2*np.pi*M_H2O))
    D_ges = 1/(1/D_Kn + 1/D_12)
    alpha = rho_s*R*T[:,i]/(eps_p*M_H2O)*delta_X_gl
    delta_eff = (D_ges/tort)/(1+alpha)
    C_kin = 15*delta_eff/((0.5*d_p)**2)
    
    X_s[:,i+1] = (X_s[:,i]+dt*C_kin*X_gl)/(1+dt*C_kin)
    ...
    Pe_d = u_leer_matrix[:,i]*d_p/D_12
    D_ax = d_bed + (Pe_d/2)*D_12

    X_f[1:-1,i+1] = X_f[1:-1,i] + dt/eps*(-(u_leer_matrix[1:-1,i]*X_f[1:-1,i]-u_leer_matrix[0:-2,i]*X_f[0:-2,i])/dz
                                      +(D_ax[1:-1]-D_ax[0:-2])/dz*(X_f[1:-1,i]-X_f[0:-2,i])/dz + D_ax[1:-1]*(X_f[2:,i]-2*X_f[1:-1,i]+X_f[0:-2,i])/(dz**2)
                                      -rho_s/rho_f_in*(1-eps)*C_kin[1:-1]*(X_gl[1:-1]-X_s[1:-1,i+1]))
    ...
    # Choose either the interpolated or fixed thermal expansion coefficient
    # beta = beta_func(T[:,i])
    ...
    C_2 = rho_s*(1-eps)*C_kin*(X_gl-X_s[:,i+1])*(h_ads+X_s[:,i+1]*dhdX)
      
    T[1:-1,i+1] = T[1:-1,i] + (dt/C_1[1:-1])*(-rho_f_in*cp_f*(u_leer_matrix[1:-1,i]*T[1:-1,i]-u_leer_matrix[0:-2,i]*T[0:-2,i])/dz 
                                              - rho_f_in*cp_w*(u_leer_matrix[1:-1,i]*T[1:-1,i]*X_f[1:-1,i]-u_leer_matrix[0:-2,i]*T[0:-2,i]*X_f[0:-2,i])/dz
